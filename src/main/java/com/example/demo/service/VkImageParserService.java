...
public class VkImageParserService {
    ...
    public Integer getDateOfPost(String groupDomain) {
        ...
        var attaches = resp.getItems().get(0).getAttachments();
        for (int i = 0; i < attaches.size(); i++) {
            if(attaches.get(i).getType().equals(WallpostAttachmentType.PHOTO)) {
                var newDate = attaches.get(i).getPhoto().getDate();
                log.info("Group: {}, date of new post: {}", groupDomain, newDate);
                return newDate;
            }
        }
        return null;
    ...
    public List<URI> getImagesLinksFromVkRepsponseQuery(GetResponse response) {
        ...
        var attaches = response.getItems().get(0).getAttachments();
        for (int i = 0; i < attaches.size(); i++) {
            if(attaches.get(i).getType().equals(WallpostAttachmentType.PHOTO)) {
